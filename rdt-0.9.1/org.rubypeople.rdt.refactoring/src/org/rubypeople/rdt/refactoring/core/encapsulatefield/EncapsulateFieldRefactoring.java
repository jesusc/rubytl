...
public class EncapsulateFieldRefactoring extends RubyRefactoring {
	...
	public EncapsulateFieldRefactoring(TextSelectionProvider selectionProvider) {
		...
		if(checker.shouldPerform()) {
			FieldEncapsulator fieldEncapsulator = new FieldEncapsulator(config);
			setEditProvider(fieldEncapsulator);
			EncapsulateFieldPage page = new EncapsulateFieldPage(fieldEncapsulator);
			pages.add(page);
		}
	}
}
