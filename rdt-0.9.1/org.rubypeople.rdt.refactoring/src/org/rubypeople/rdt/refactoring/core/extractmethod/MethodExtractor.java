...
public class MethodExtractor extends MultiEditProvider {
	...
	protected Collection<EditProvider> getEditProviders() {
		...
		providers.add(new ExtractedMethodCall(config));
		providers.add(new ExtractedMethodDef(config));

		return providers;
	}

	public EditProvider getDefEdit() {
		return new ExtractedMethodDef(config);
	}

	public void setMethodName(String name) {
		config.getHelper().setMethodName(name);
	}

	public ExtractedMethodHelper getExtractedMethod() {
		return config.getHelper();
	}
}
