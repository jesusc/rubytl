...
public class MoveMethodRefactoring extends RubyRefactoring {
	...
	public MoveMethodRefactoring(TextSelectionProvider selectionProvider) {
		...
		if(checker.shouldPerform()) {
			MethodMover mover = new MethodMover(config);
			setEditProvider(mover);
			pages.add(new FirstMoveMethodPage(config));
			if(config.needsSecondPage()) {
				SecondMoveMethodPage secondPage = new SecondMoveMethodPage(config);
