...
public class InlineMethodRefactoring extends RubyRefactoring {
	...
	public InlineMethodRefactoring(TextSelectionProvider selectionProvider) {
		...
		if(checker.shouldPerform()) {
			InlineAndRemoveEditProvider removeEditProvider = new InlineAndRemoveEditProvider(config);
			setEditProvider(removeEditProvider);
			pages.add(new InlineMethodPage(removeEditProvider));
		}
	}
}
