...
//		viewer.addDropSupport(ops, transfers, new DelegatingDropAdapter(dropListeners));
	...
	public void createPartControl(Composite container) {
		...
		IRubyElement input= null; //determineInputElement();
		if (fMemento != null) {
		...
		PlatformUI.getWorkbench().getHelpSystem().setHelp(fPagebook, IRubyHelpContextIds.TYPE_HIERARCHY_VIEW);
...
//				new RefactorActionGroup(this),
	...
	private void updateTitle() {
		...
		if (fInputElement != null) {
			...
			if (workingSet == null) {
				String[] args= new String[] { viewerTitle, RubyElementLabels.getElementLabel(fInputElement, RubyElementLabels.ALL_DEFAULT) };
				...
				String[] args= new String[] { viewerTitle, RubyElementLabels.getElementLabel(fInputElement, RubyElementLabels.ALL_DEFAULT), workingSet.getLabel() };
	...
	public void saveState(IMemento memento) {
		...
		IRubyElement selection= (IRubyElement)((IStructuredSelection) getCurrentViewer().getSelection()).getFirstElement();
		if (selection != null) {
	...
	private void restoreState(final IMemento memento, IRubyElement defaultInput) {
		...
		if (elementId != null) {
			input= RubyCore.create(elementId);
