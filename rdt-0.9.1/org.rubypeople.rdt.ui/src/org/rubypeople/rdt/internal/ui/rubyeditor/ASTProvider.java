...
public final class ASTProvider {
	...
	private void activeRubyEditorChanged(IWorkbenchPart editor) {
		...
		if (editor instanceof RubyEditor)
			javaElement= ((RubyEditor)editor).getInputRubyElement();

		synchronized (this) {
			fActiveEditor= editor;
			fActiveRubyElement= javaElement;
		...
		synchronized (fReconcileLock) {
			if (fIsReconciling && (fReconcilingRubyElement == null || !fReconcilingRubyElement.equals(javaElement))) {
	 ...
	 * @see org.eclipse.jdt.internal.ui.text.java.IRubyReconcilingListener#aboutToBeReconciled()
	 */
	void aboutToBeReconciled(IRubyElement javaElement) {
		...
		synchronized (fReconcileLock) {
			fIsReconciling= true;
			fReconcilingRubyElement= javaElement;
		}
		cache(null, javaElement);
	}
	 ...
	 * @param javaElement
	 */
	private synchronized void cache(Node ast, IRubyElement javaElement) {
	 ...
	 * </p>
	 *
	 * @param je				the Ruby element
	 * @param waitFlag			{@link #WAIT_YES}, {@link #WAIT_NO} or {@link #WAIT_ACTIVE_ONLY}
	 * @param progressMonitor	the progress monitor or <code>null</code>
	 * @return					the AST or <code>null</code> if the AST is not available
	 */
	public Node getAST(IRubyElement je, WAIT_FLAG waitFlag, IProgressMonitor progressMonitor) {
		if (je == null)
			return null;
		
		Assert.isTrue(je.getElementType() == IRubyElement.SCRIPT);
		...
		synchronized (this) {
			isActiveElement= je.equals(fActiveRubyElement);
			if (isActiveElement) {
				if (fAST != null) {
					if (DEBUG)
	 ...
	 * @return AST
	 */
	private Node createAST(IRubyElement je, final IProgressMonitor progressMonitor) {
		if (!hasSource(je))
			return null;
		...
		SafeRunner.run(new ISafeRunnable() {
			public void run() {
				try {
					if (progressMonitor != null && progressMonitor.isCanceled())
						root[0]= null;
					root[0]= parser.parse(goodSource);
...
//			ASTNodes.setFlagsToAST(root[0], ASTNode.PROTECT);
	 ...
	 * 
	 * @param je the Ruby element to test
	 * @return <code>true</code> if the element has source
	 * @since 3.2
	 */
	private boolean hasSource(IRubyElement je) {
		...
		try {
			...
			IStatus status= new Status(IStatus.ERROR, RubyUI.ID_PLUGIN, IStatus.OK, "Error in JDT Core during AST creation", ex);  //$NON-NLS-1$
			RubyPlugin.getDefault().getLog().log(status);
		}
		return false;
	}
	 ...
	 */
	void reconciled(Node ast, IRubyElement javaElement, IProgressMonitor progressMonitor) {
