 ...
 * <code>org.rubypeople.rdt.ui.queryParticipants</code>.
 * <p> 
...
public interface IQueryParticipant {
	 ...
	 * The interpretation of what a given Ruby search (e.g. "References to class Foo") means is up to the 
	 * participant.
	 ...
	 * that the number of ticks for the original Ruby search job is 1000. For example if the participant
	 * uses the same amount of time as the Ruby search, it should return 1000, if it uses half the time,
	...
	/**
	 * Gets the UI participant responsible for handling the display of elements not known to the Ruby search UI.  The Java search UI knows
	 * elements are of type <code>IRubyElement</code> and <code>IResource</code>.
	 * A participant may return <code>null</code> if matches are only reported against elements of type <code>IResource</code> and <code>IRubyElement</code>.
