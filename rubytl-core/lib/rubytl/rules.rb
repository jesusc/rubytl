  ...
  module Rtl
    ...
    class Rule
      ...
      def target_conforms_to?(target_class)
        self.to_part.types.all? { |c| 
          c.rumi_conforms_to?(target_class) 
        }
      end
    ...
    class MetaRule 
      ...
      attr_reader :kind
      attr_accessor :filter_part
      attr_accessor :behaviour_part

      BINDING_KIND = :binding
      VALID_KINDS  = [BINDING_KIND]
      def initialize(name, kind)
        raise InvalidMetaRuleKind.new("Invalid kind of meta rule #{kind}") unless VALID_KINDS.include?(kind)
        @name = name
        @kind = kind
      end

      def binding_applied(binding, new_instances)
         puts new_instances
      end
    end
    ...
    class MetaBehaviourPart < CodePart
    end   
    ...
    class InvalidMetaRuleKind < RubyTL::BaseError; end 
