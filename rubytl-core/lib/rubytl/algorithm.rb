  ...
  module Rtl
    ...
    class Algorithm
      ...
      def resolve_binding(binding)
        top_level_transformation = @transformation.transformation
        binding_applied_notification = if top_level_transformation.binding_meta_rules.size > 0
          lambda { |b, r| top_level_transformation.binding_meta_rules.each { |m| m.binding_applied(b, r) } }
        end
        binding.homogenize_enumerable_binding do |binding|                               
          if not binding.is_source_nil?
            if binding.directly_assignable? 
              ...
              binding_applied_notification.call(binding, binding.target_instance) if binding_applied_notification
            else
              selected = self.select_conforming_rules(binding, transformation.rules)
              self.evaluate_available_rules(binding, selected, binding_applied_notification)  
            end
          end
        end      
      end
      ...
      # 
      def evaluate_available_rules(binding, selected, binding_applied_notification = nil)
        ...
        results = selected.map do |rule|
          ...
          results = rule.apply(binding.tupleized_source, new_instances)        
          binding_applied_notification.call(binding, new_instances) if binding_applied_notification
          results
        end.flatten
      end
      ...
      def select_entry_point_rules
        log_event(TLogger::EmptyPhase, transformation.name) if (transformation.init_rules + transformation.rules).empty?
        selected = transformation.init_rules + transformation.rules.select { |r| r.kind_of?(TopRule) }        
        if selected.empty?
