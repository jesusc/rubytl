  ...
  module Base  
    module ExceptionHandling
      ...
      def handle_dsl_exceptions(filenames)
        ...
        raise_custom RubyTL::RubySyntaxError.new(e, [*filenames])   
      rescue RubyTL::InvalidHelper => e
        raise_custom RubyTL::RubySyntaxError.new(e, [*filenames])
      rescue RMOF::FeatureNotExist => e
        raise_custom RubyTL::FeatureNotExistError.new(e, [*filenames])
      rescue RMOF::TypeCheckError => e
        raise_custom RubyTL::EvaluationError.new(e, [*filenames])
      ...
      def raise_custom(ex)
        if RUBY_VERSION < '1.9'
          ...
          if ex.kind_of?(RubyTL::EvaluationError)
            # This is a workaround to pass parameters to the exception
            # in Ruby 2.0... 
            e = ex.wrapped
            e.instance_variable_set("@filenames", ex.filenames)
            raise ex.class, e
          else
            raise ex
          end
      ...
      def handle_repository_exceptions(model)       
        ...
        puts e.backtrace
          raise_custom RubyTL::ModelLoadingError.new(e, model)
        end
      end
...
#    def match_filename?(original_filename, element)
  ...
  class EvaluationError < BaseError
    ...
    def initialize(e, filenames = nil, options = {})      
      ...
      if filenames.nil?
        # Try Ruby 2.0 style...
        filenames = e.instance_variable_get("@filenames")
      end
