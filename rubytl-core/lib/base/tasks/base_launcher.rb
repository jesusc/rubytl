  ...
  module Base
    ...
    class Launcher
      ...
      def evaluate
        ...
        benchmark_if_flag('Model loading', @benchmark.include?(:model_loading)) {
          context = evaluate_dsl_only
        }
        benchmark_if_flag('Execution', @benchmark.include?(:execution)) {
          execute_dsl(context)
        }
      ...
      def serialize_target_models(target_models)
        benchmark_if_flag('Serialization', @benchmark.include?(:serialization)) do
          target_models.select { |m| m.serializable? }.each do |model|
            model.serialize(@config.serializer_options)
          end
      ...
      def benchmark_if_flag(name, exec, &block) 
        require 'benchmark'
        if exec
          puts name
